function fsm_gui_orientation_whisker()
close all
...
fsm.handles.soundvis = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','checkbox',...
    'Position', [0.35 0.25 0.12 0.04],'String','Whisker + Ori?',...
    'Value',1,'FontSize',10);
...
while keeprunning 
    ...
    if max(strcmp(fsm.stimmodality{fsm.trialnum+1}, fsm.attentionlocationlist([1]))) % vis
        Cont = 5;Rew = 7;
    elseif max(strcmp(fsm.stimmodality{fsm.trialnum+1}, fsm.attentionlocationlist([2]))) % whisker
        Cont = 6;Rew = 8;
    else
        error('somethings wrong')
    end
    ...
    switch rcvd
        ...
        case 'startingFSM'
            ...
            while trialend == 0%~strcmp(rcvd2,'E'); % it will send an E when trial ends
                ...
                stim_machine_LR_orientation_whisker
