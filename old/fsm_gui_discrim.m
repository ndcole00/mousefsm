...
fsm.Tcue = 0;
fsm.Tstimdelaymin = 1;
fsm.Tstimdelaymeanadd = 0;
fsm.Tcontrastdelaymin = .5;
fsm.Tcontrastdelaymeanadd = 0;
fsm.Tmincontrastview = .4;
...
uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit','Enable','inactive',...
    ...
    'String','P rewd trial','FontSize',10);
fsm.handles.prewdtrial = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit',...
    'Position', [0.23 0.1 0.1 0.04],...
    'String',fsm.prewdtrial,'FontSize',10);
...
% Early lick ok?
fsm.handles.earlylickok = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','checkbox',...
    'Position', [0.35 0.25 0.12 0.04],'String','Early lick ok?',...
    'Value',0,'FontSize',10);

% Ang min view time (extra ITI will be added to this)
uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit','Enable','inactive',...
    'Position', [0.45 0.25 0.1 0.04],...
    'String','Ang view T','FontSize',10);
fsm.handles.AviewT = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit',...
    'Position', [0.56 0.25 0.05 0.04],...
    'String',fsm.AviewT,'FontSize',10);
...
while keeprunning 
    ...
    avt   = str2num(get(fsm.handles.AviewT,'string'));
    ...
    if get(fsm.handles.earlylickok ,'Value')==1
        lok = 5;% don't restart on early lick
    else
    ...
    if max(strcmp(fsm.checkerboardlocation{fsm.trialnum+1}, fsm.attentionlocationlist([1 3]))) % left (or front)
        %Cont = 5;Rew = 7;
        if fsm.orientation(fsm.trialnum+1) == 0 % vertical rewarded
            Chk = 5; Rew = 7; Stp = 10; 
        elseif fsm.orientation(fsm.trialnum+1) == 45 % angled non rewarded
            Chk = 9; Rew = 0; Stp = 11; 
        else error('here')
        end
    elseif max(strcmp(fsm.checkerboardlocation{fsm.trialnum+1}, fsm.attentionlocationlist([2 4]))) % right (or back)
        %Cont = 6;Rew = 8;
        if fsm.orientation(fsm.trialnum+1) == 0 % vertical rewarded
            Chk = 6; Rew = 8; Stp = 10;
        elseif fsm.orientation(fsm.trialnum+1) == 45 % angled non rewarded
    ...
    % 4-stim both sides; 5-checkerboardVert left; 6-checkerboardVert right
    % 7-rewd+checkerboardVert left; 8-rewd+checkerboardVert right;
    % 9-checkerboardAng left; 10-checkerboardAng right
    ...
    %  spd in   spd out     lick    Tup       Timer   digiOut
        ...
        4           1        4       5         stimD    0               ;...% state 4 wait for stim
        5           1        lok     Stp       stimT    Stim            ;...% state 5 stim on, wait for checkerboard
        6           6        7       7         waitT    Chk             ;...% state 6 Vert checkerboard, wait for lick
        7           7        7       8         rewT     Rew             ;...% state 7 reward on
        8           8        8       9         extraT   Chk             ;...% state 8 extra ITI, used to keep stim on 
        9           9        9       99        iti      0               ;...% state 9 ITI
        10          10       10      6         mcvT     Chk             ;...% state 10 min time forced to look at vert checkerboard
        11          11       12      9         avt      Chk             ;...% state 11 show angled checkerboard, lick goes to punishment/forced looking
        12          12       12      13        .1       11              ;...% state 12 puff
        13          13       13      11        3        Chk             ;...% state 13 forced looking
        %14          14       14      15        .2       Chk             ;...% state 12 flash
        %15          15       15      16        .2       0               ;...% state 12 flash
        %16          16       16      17        .2       Chk             ;...% state 12 flash
        %17          17       17      18        .2       0               ;...% state 12 flash
        %18          18       18      19        .2       Chk             ;...% state 12 flash
        %19          19       19      13        .2       0               ;...% state 12 flash
        ];
            ...
            while trialend == 0%~strcmp(rcvd2,'E'); % it will send an E when trial ends
                ...
                if fsm.ard.BytesAvailable
                    ...
                    if rcvd2 == 'E';
                        ...
                        fsm.RT(fsm.trialnum) = findreactiontime(fsm.triallog{fsm.trialnum});
                        updateRTplot
...
fsm.spdxrng = str2num(get(fsm.handles.spdxrng,'string'));
 ...
 function RT = findreactiontime(triallog)
     % vert trial 5 to 6, lick 6 to 7
     global fsm
     if fsm.orientation(fsm.trialnum) == 0 % vertical rewarded
     ...
     else % angled nonrewarded
         ct1 = strfind(triallog, '_5to11');
         ct2 = strfind(triallog(1:ct1(end)-1),'_');
         ct = str2num(triallog(ct2(end)+1:ct1(end)-1));
         lt1 = strfind(triallog, '_11to12');
         if ~isempty(lt1)
             lt2 = strfind(triallog(1:lt1(1)-1),'_');
             lt = str2num(triallog(lt2(end)+1:lt1(1)-1));
             RT = (lt-ct)/1000;
         else
             RT = NaN;
         end
...
%else
mrkr = '.';
    switch fsm.orientation(fsm.trialnum)
        case 0
            cmrkr = 'b';
        case 45
            cmrkr = 'r';
...
% now choose checkerboard location
if rand < str2num(get(fsm.handles.pmismatch,'String')) % mismatch trial
    ...
    if strcmp(fsm.attentionlocation{fsm.trialnum+1},fsm.attentionlocationlist{1})
        fsm.checkerboardlocation{fsm.trialnum+1} = fsm.attentionlocationlist{2}; 
    elseif strcmp(fsm.attentionlocation{fsm.trialnum+1},fsm.attentionlocationlist{2})
        fsm.checkerboardlocation{fsm.trialnum+1} = fsm.attentionlocationlist{1};
    elseif strcmp(fsm.attentionlocation{fsm.trialnum+1},fsm.attentionlocationlist{3})
        fsm.checkerboardlocation{fsm.trialnum+1} = fsm.attentionlocationlist{4};
    elseif strcmp(fsm.attentionlocation{fsm.trialnum+1},fsm.attentionlocationlist{4})
        fsm.checkerboardlocation{fsm.trialnum+1} = fsm.attentionlocationlist{3};
    end
else % no mismatch
    fsm.mismatch(fsm.trialnum+1) = 0;
    set(fsm.handles.mismatch,'String',['Mismatch trial: ' num2str(fsm.mismatch(fsm.trialnum+1))]);
    fsm.checkerboardlocation{fsm.trialnum+1} = fsm.attentionlocation{fsm.trialnum+1}; 
end
    
% Now choose the orientation from 2 options, vert (rewarded) or ang (non rewarded)
if rand < str2num(get(fsm.handles.prewdtrial,'String')) % rewd trial
    fsm.orientation(fsm.trialnum+1) = 0;
else
    fsm.orientation(fsm.trialnum+1) = 45;
end
