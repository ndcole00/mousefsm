function fsm_gui_orientation_odour_ignore()
close all
...
fsm.Torientationdelaymin = 1.5;
fsm.Torientationdelaymeanadd = 2;
fsm.Tminorientationview = .2;
fsm.Trewdavailable = 1;
fsm.Titi = .1;
fsm.contrast = .5;
fsm.orientation = [];
fsm.orientationchange = 45;
fsm.spatialfreq = .1;
fsm.temporalfreq = 2;
...
uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit','Enable','inactive',...
    ...
    'String','orientation change deg','FontSize',10);
fsm.handles.orientationchange = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit',...
    'Position', [0.23 0.1 0.1 0.04],...
    'String',fsm.orientationchange,'FontSize',10);
...
while keeprunning 
    ...
    elseif max(strcmp(fsm.stimmodality{fsm.trialnum+1}, fsm.attentionlocationlist([2]))) % odour
        Cont = 6; 
        Rew = 8;
        if fsm.irreltrial(fsm.trialnum+1) == 1;Cont = 12; Rew = 1;ttp = 12; else ttp = 10;end % irrel cont change
    else
        error('somethings wrong')
    end
    IrrC = 5; % Irrelvant orientation change
    
    stm = [... % remember zero indexing; units are seconds, multiplied later to ms
        
    % digiOut mapping: 1-rewd valve; 2-vis cue; 3-sound cue; 
    % 4-stim both sides; 5-vis orientation change; 6-sound
    % 7-rewd+vishigh cont; 8-rewd + vis low orientation
    ...
    %  spd in   spd out     lick    Tup       Timer   digiOut
        ...
        5           5        lok2    ttp       stimT    Stim            ;...% state 5 stim on, wait for orientation change
        6           6        7       9         waitT    Cont            ;...% state 6 orientation change, wait for lick
    ...
    switch rcvd
        ...
        case 'startingFSM'
            ...
            while trialend == 0%~strcmp(rcvd2,'E'); % it will send an E when trial ends
                ...
                stim_machine_LR_orientation_odour
...
global fsm
% orientation change 5 to 10 normal trial, 12 to 10 irrel trial, lick 6 to 7
