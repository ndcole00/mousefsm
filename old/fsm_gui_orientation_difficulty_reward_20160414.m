...
%     end
    ...
    %  spd in   spd out     lick    Tup       Timer   digiOut
        ...
        4           4        lok     5         stimD    Cu2               ;...% state 4 wait for stim
...
switch blockORtbtRewd % rewd
    case 1 % blocks
        ...
        % is it time to change blocks?
        if fsm.trialnum+1 - fsm.blockchangetrialRewd >= str2num(get(fsm.handles.ntrialsperblockRewd,'String'))
            fsm.blockchangetrialRewd = fsm.trialnum+1;
            %curr = find(fsm.difflist==fsm.orientationchange(fsm.trialnum));
