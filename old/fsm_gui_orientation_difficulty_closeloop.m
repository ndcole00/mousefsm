...
function fsm_gui_orientation_difficulty_closeloop()
close all
...
fsm.Torientationdelaymin = 60;
fsm.Torientationdelaymeanadd = 40;
fsm.Tminorientationview = .25;
fsm.Trewdavailable = 30;
fsm.Titi = 1;
fsm.contrast = 1;
fsm.orientationchange = [];
fsm.orientationchangelist = '45';
fsm.orientation = [];
fsm.spatialfreq = .1;
fsm.temporalfreq = 2;
...
stim_machine_LR_init_closeloop
    ...
    switch rcvd
        ...
        case 'startingFSM'
            ...
            while trialend == 0%~strcmp(rcvd2,'E'); % it will send an E when trial ends
                ...
                stim_machine_LR_orientation_closeloop_freezetopunish
...
global fsm
if isnan(fsm.RT(end)); RT = 1; else RT = fsm.RT(end);end
...
% Now choose the orientation from 3 options, go through list without replacement
if rem(fsm.trialnum,3) == 0
    fsm.orilist = randperm(3);
end
fsm.orientation(fsm.trialnum+1) = (fsm.orilist(rem(fsm.trialnum,3)+1))*360/8 + 90;
set(fsm.handles.orientation, 'String',['Orientation: ' num2str(fsm.orientation(fsm.trialnum+1))]);
