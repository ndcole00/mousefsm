function fsm_gui_contrast_odour_ignore()
close all
...
fsm.Tcontrastdelaymin = 1.5;
fsm.Tcontrastdelaymeanadd = 2;
fsm.Tmincontrastview = .2;
fsm.Trewdavailable = 1;
fsm.Titi = .1;
fsm.contrast = .2;
fsm.contrastchange = 200;
fsm.orientation = [];
fsm.spatialfreq = .1;
fsm.temporalfreq = 2;
fsm.attentionlocationlist = {'vis','odour'};
fsm.attentionlocation = {};
fsm.stimmodality = {};
fsm.pirrel = 0;
fsm.ntrialsperblock = 200;
fsm.rewd = .1;
fsm.trialnum = 0;
fsm.triallog = {};
fsm.lickthreshold = .25;
fsm.RT = [];
...
fsm.handles.ax(3)=axes('Parent',fsm.handles.f,'Units','normalized','Position',[0.35 -.03 0.27 0.3]);
...
uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit','Enable','inactive',...
    ...
    'String','Prob. irrel trials','FontSize',10);
...
% Min contrast view time
uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit','Enable','inactive',...
    'Position', [0.35 0.45 0.17 0.04],...
    'String','Min contrast view T','FontSize',10);
fsm.handles.Tmincontrastview = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit',...
    'Position', [0.53 0.45 0.08 0.04],...
    'String',fsm.Tmincontrastview,'FontSize',10);
...
% lick to irrel grating ok?
fsm.handles.lickok = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','checkbox',...
    'Position', [0.35 0.25 0.12 0.04],'String','lick ok?',...
    'Value',1,'FontSize',10);
...
fsm.handles.attentionlocation = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit','Enable','inactive',...
    ...
    'String',['Stim location: ' fsm.attentionlocationlist{2}],'FontSize',10,'HorizontalAlignment','left');

% Irrel trial
fsm.handles.irrel = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit','Enable','inactive',...
    'Position', [0.35 0.75 0.26 0.04],...
    'String',['Irrel trial: ' ],'FontSize',10,'HorizontalAlignment','left');
...
% Min Irrel duration (rand*.5 added)
uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit','Enable','inactive',...
    'Position', [0.35 0.7 0.17 0.04],...
    'String','Irrel Duration','FontSize',10);
fsm.handles.Irreldur = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit',...
    'Position', [0.53 0.7 0.08 0.04],...
    'String',fsm.Irreldur,'FontSize',10);
...
while keeprunning 
    ...
    stimT = str2num(get(fsm.handles.Tcontrastdelaymin,'string')) + rand*str2num(get(fsm.handles.Tcontrastdelaymeanadd,'string'));
    ...
    mcvT  = str2num(get(fsm.handles.Tmincontrastview,'string'));
    pT    = str2num(get(fsm.handles.punishT,'string'));
    fsm.contrast = str2num(get(fsm.handles.contrast,'string'));
    fsm.contrastchange = str2num(get(fsm.handles.contrastchange,'String'));
    if ~fsm.iscuetrial ; cueT = 0; end
    icT   = str2num(get(fsm.handles.Irreldur,'string')) + rand*.5;
    ...
    if get(fsm.handles.lickok,'Value')==1 % if lick ok to irrel cont change
        lok4 = 13;lok3 = 10;
    else
        lok4 = 11;lok3 = 11;% punish on early lick
    end
    
    Pun = 9; %Punish - pause the drifting grating
    
    %Rew = 1;
    Stim = 4;
    if max(strcmp(fsm.attentionlocation{fsm.trialnum+1}, fsm.attentionlocationlist([1]))) % vis
        Cue = 2;
    elseif max(strcmp(fsm.attentionlocation{fsm.trialnum+1}, fsm.attentionlocationlist([2]))) % sound
        Cue = 3;
    else
    ...
    if max(strcmp(fsm.stimmodality{fsm.trialnum+1}, fsm.attentionlocationlist([1]))) % vis
        Cont = 5;Rew = 7;ttp = 10;
    elseif max(strcmp(fsm.stimmodality{fsm.trialnum+1}, fsm.attentionlocationlist([2]))) % odour
        Cont = 6;Rew = 8;
        if fsm.irreltrial(fsm.trialnum+1) == 1; ttp = 12; else ttp = 10;end % irrel cont change
    else
        error('somethings wrong')
    end
    IrrC = 5; % Irrelvant contrast change
    
    stm = [... % remember zero indexing; units are seconds, multiplied later to ms
        
    % digiOut mapping: 1-rewd valve; 2-vis cue; 3-sound cue; 
    % 4-stim both sides; 5-vis contrast change; 6-sound
    % 7-rewd+vishigh cont; 8-rewd + vis low contrast
    ...
    %  spd in   spd out     lick    Tup       Timer   digiOut
        ...
        5           5        lok2    ttp       stimT    Stim            ;...% state 5 stim on, wait for contrast change
        6           6        7       9         waitT    Cont            ;...% state 6 contrast change, wait for lick
        7           7        7       8         rewT     Rew             ;...% state 7 reward on
        8           8        8       9         extraT   Cont            ;...% state 8 extra ITI 
        9           9        9       99        iti      0               ;...% state 9 ITI
        10          10       lok3    6         mcvT     Cont            ;...% state 10 min time forced to look at changed contrast
        11          11       11      1         pT       0               ;...% punishT
        12          12       lok4    10        icT      IrrC            ;...% Irrel cont change in odour trial
        13          13       13      10        icT      IrrC            ;...% To record the lick for RT
    ...
    switch rcvd
        ...
        case 'startingFSM'
            ...
            while trialend == 0%~strcmp(rcvd2,'E'); % it will send an E when trial ends
                ...
                stim_machine_LR_contrast_odour
...
global fsm
% contrast change 5 to 10 normal trial, 12 to 10 irrel trial, lick 6 to 7
ct1 = strfind(triallog, '_5to10');
if isempty(ct1); ct1 = strfind(triallog, '_12to10');end % if irrel trial
if isempty(ct1); ct1 = strfind(triallog, '_13to10');end % if irrel trial
...
if fsm.irreltrial(fsm.trialnum) ==1 % if irrel trial, find RT from cont change
    ict1 = strfind(triallog, '_5to12');
    ict2 = strfind(triallog(1:ict1(end)-1),'_');
    ict = str2num(triallog(ict2(end)+1:ict1(end)-1));
    
   
    if get(fsm.handles.lickok, 'Value') == 1
         ilt1 = strfind(triallog, '_12to13');
    else
         ilt1 = strfind(triallog, '_12to11');
    end
    if ~isempty(ilt1)
        ilt2 = strfind(triallog(1:ilt1(end)-1),'_');
        ilt = str2num(triallog(ilt2(end)+1:ilt1(end)-1));
        fsm.RT2 = (ilt-ict)/1000;
    else
        fsm.RT2 = NaN;
    end
end
...
mrkr = '.';
    ...
    if fsm.irreltrial(fsm.trialnum) ==1 % if irrel trial, plot RT from cont change
         plot(fsm.handles.ax(2),fsm.trialnum,fsm.RT2,'color',[.2 .2 .2],'marker',mrkr);
    end
        ...
        switch blockORtbt
            case 1 % blocks
                ...
                if fsm.trialnum+1 - fsm.blockchangetrial >= str2num(get(fsm.handles.ntrialsperblock,'String'))
                    ...
                    if strcmp(fsm.attentionlocation{fsm.trialnum}, fsm.attentionlocationlist{1});%if its 'vis'
                        fsm.attentionlocation{fsm.trialnum+1} = fsm.attentionlocationlist{2};% make it 'sound'
                    else
                        fsm.attentionlocation{fsm.trialnum+1} = fsm.attentionlocationlist{1};% make it 'vis'
                    end
                else
                    if fsm.trialnum == 0 % if first trial
                        fsm.attentionlocation{fsm.trialnum+1} = fsm.attentionlocationlist{2};
                    else
                        fsm.attentionlocation{fsm.trialnum+1} = fsm.attentionlocation{fsm.trialnum};
                    end
                end
                
                % Now choose if you want to give a cue or not
                if fsm.trialnum+1 - fsm.blockchangetrial >= str2num(get(fsm.handles.ntrialswithcue,'String'))
                    fsm.iscuetrial=0;else fsm.iscuetrial=1;
                end
                
            case 2 % trial by trial
                % choose left or right
                if rand>.5
                    fsm.attentionlocation{fsm.trialnum+1} = fsm.attentionlocationlist{1};% vis
                else
                    fsm.attentionlocation{fsm.trialnum+1} = fsm.attentionlocationlist{2};% sound
                end
        end
%     case 2 % front/back
%         % find out if blockwise or trial by trial
...
%                 
%                 % Now choose if you want to give a cue or not
%                 if fsm.trialnum+1 - fsm.blockchangetrial >= fsm.ntrialswithcue
%                    fsm.iscuetrial=0;else fsm.iscuetrial=1;
%                 end
%                 
%             case 2 % trial by trial
...
% no mismatch in this version, only irrel
% now choose stim change vis or sound
if strcmp(fsm.attentionlocation{fsm.trialnum+1},  fsm.attentionlocationlist{2}) % if odr trial
    if rand < str2num(get(fsm.handles.pirrel,'String')) % irrel trial
        fsm.irreltrial(fsm.trialnum+1) = 1;
        set(fsm.handles.irrel,'String',['Irrel trial: ' num2str(fsm.irreltrial(fsm.trialnum+1))]);
    else % no irrel
        fsm.irreltrial(fsm.trialnum+1) = 0;
        set(fsm.handles.irrel,'String',['Irrel trial: ' num2str(fsm.irreltrial(fsm.trialnum+1))]);
    end
else
    fsm.irreltrial(fsm.trialnum+1) = 0;
...
fsm.stimmodality{fsm.trialnum+1} = fsm.attentionlocation{fsm.trialnum+1}; 
