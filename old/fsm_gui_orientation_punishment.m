function fsm_gui_orientation_punishment()
close all
...
fsm.Torientationdelaymin = .5;
fsm.Torientationdelaymeanadd = 0;
...
while keeprunning 
    ...
    if max(strcmp(fsm.orientationchangelocation{fsm.trialnum+1}, fsm.attentionlocationlist([1 3]))) % left or front
        Cont = 5;Rew = 7;
    elseif max(strcmp(fsm.orientationchangelocation{fsm.trialnum+1}, fsm.attentionlocationlist([2 4]))) % right or back
        Cont = 6;Rew = 8;
    else
        error('somethings wrong')
    end
    ...
    % 7-rewd+ContL; 8-rewd+ContR; 9-white screen punishment
    ...
    %  spd in   spd out     lick    Tup       Timer   digiOut
        ...
        5           1        lok     10        stimT    Stim            ;...% state 5 stim on, wait for orientation change
    ...
    switch rcvd
        ...
        case 'startingFSM'
            ...
            while trialend == 0%~strcmp(rcvd2,'E'); % it will send an E when trial ends
                ...
                stim_machine_LR_orientation_punishment
