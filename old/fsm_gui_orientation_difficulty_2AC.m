function fsm_gui_orientation_difficulty_2AC()
close all
...
fsm.orientationchangelist = '45,30';
fsm.orientation = [];
fsm.spatialfreq = .1;
fsm.temporalfreq = 2;
fsm.changelocationlist = {'left','right'};
...
uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit','Enable','inactive',...
    ...
    'String','Prob. Left trials','FontSize',10);
fsm.handles.pLeft = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit',...
    'Position', [0.23 0.25 0.1 0.04],...
    'String',fsm.pLeft,'FontSize',10);
...
fsm.handles.orientationchangelocation = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit','Enable','inactive',...
   'Position', [0.35 0.75 0.26 0.04],...
   'String',['Change location: ' ],'FontSize',10,'HorizontalAlignment','left');
...
% which side will change and which lick is rewarded
    if isequal(fsm.orientationchangelocation{fsm.trialnum+1},'left')
        Cont = 5;Rew = 1;% rewd valve is L
        LL = 7; % go to rewd state
        LR = 11; % go to punish state
    elseif isequal(fsm.orientationchangelocation{fsm.trialnum+1},'right')
        Cont = 6;Rew = 7;% rewd valve is R
        LL = 11; % go to punish state
        LR = 7; % go to rewd state
    end
    %set(fsm.handles.bothsides ,'Value',1)
    
    stm = [... % remember zero indexing; units are seconds, multiplied later to ms
        
    % digiOut mapping: 1-rewd valveL; 2-left cue; 3-right cue; 
    % 4-stim both sides; 5-left orientation change; 6-right orientation change
    % 7-rewdL+ContL; 8-rewdL+ContR
    % 9-rewd valveR; 10-rewdR+ContL; 11-rewdR+ContR; 12,13 free
        
    %  spd in   spd out     lickL     lickR     Tup       Timer   digiOut
        0           0        0         0         1         0.01     0               ;...% state 0 init
        2           1        1         1         1         100      0               ;...% state 1 wait for speed in
        2           1        2         2         3         cueD     0               ;...% state 2 wait for cue
        3           1        3         3         4         cueT     Cue             ;...% state 3 cue on
        4           1        lok       lok       5         stimD    0               ;...% state 4 wait for stim
        5           5        lok2      lok2      10        stimT    Stim            ;...% state 5 stim on, wait for orientation change
        6           6        LL        LR        9         waitT    Cont            ;...% state 6 orientation change, wait for lick
        7           7        7         7         8         rewT     Rew             ;...% state 7 reward on
        8           8        8         8         9         extraT   Cont            ;...% state 8 extra ITI to view grating while drinking
        9           9        9         9         99        iti      0               ;...% state 9 ITI
        10          10       lok3      lok3      6         mcvT     Cont            ;...% state 10 min time forced to look at changed orientation
        11          11       11        11        1         pT       0               ;...% punishT
        
        ];

    stm (:,6) = round(stm(:,6)*1000); % sec to ms % change this if adding a column
...
%else
difflist = str2num(get(fsm.handles.orientationchangelist,'String'));
curr = find(difflist==fsm.orientationchange(fsm.trialnum));
mrkr = '.';
    switch curr
        case 1
            cmrkr = 'b';
    ...
    if isnan(fsm.RT(end)); % miss without mismatch
        mrkr = '^';
    end
...
if isempty(fsm.orientationchange) % first trial
    fsm.orientationchange(1) = difflist(1);
end
blockORtbt = get(fsm.handles.blockORtbt,'Value');
switch blockORtbt % choose difficulty
    case 1 % blocks
        ...
        if fsm.trialnum+1 - fsm.blockchangetrial >= str2num(get(fsm.handles.ntrialsperblock,'String'))
            ...
            curr = find(difflist==fsm.orientationchange(fsm.trialnum));
            if curr == length(difflist)
                fsm.orientationchange(fsm.trialnum+1) = difflist(1);
            else
                fsm.orientationchange(fsm.trialnum+1) = difflist(curr+1);
            end
    ...
    case 2 % trial by trial
        rr = randi([1, length(difflist)]);
        fsm.orientationchange(fsm.trialnum+1) = difflist(rr);
end
...
if rand < str2num(get(fsm.handles.pLeft,'String')) % p left/right change
    fsm.orientationchangelocation{fsm.trialnum+1} = fsm.changelocationlist{1};% Left
else
    fsm.orientationchangelocation{fsm.trialnum+1} = fsm.changelocationlist{2}; % Right
end
set(fsm.handles.orientationchangelocation,'String',['Orientation change loc: ' fsm.orientationchangelocation{fsm.trialnum+1}])
