% 20042016 left monitor is always 2, right is 1

function stim_machine_LR_init_sound

global fsm


fsm.s = daq.createSession('ni');
fsm.s.addDigitalChannel('Dev1', 'Port0/Line1:7', 'InputOnly');%include 0 to make it an input line
...
try
    ...
    fsm.snd = cos(2*pi*1000*t)+1/2*sin(2*pi*2000*(t-pi/4))+1/4*cos(2*pi*4000*t);
    fsm.snd = fsm.snd/max(fsm.snd);
    InitializePsychSound;
    deviceid = -1;
    reqlatencyclass = 2;
    
    freq = 8192;%44100;       % Must set this. 
    buffersize = 0;     % Pointless to set this. Auto-selected to be optimal.
    suggestedLatencySecs = [];
    % Open audio device for low-latency output:
%     fsm.pahandle = PsychPortAudio('Open', deviceid, [], reqlatencyclass, freq, 1, buffersize, suggestedLatencySecs);
    % Fill buffer with data:
%     PsychPortAudio('FillBuffer', fsm.pahandle, fsm.snd);
    ...
    [fsm.winR,screenRect] = Screen('OpenWindow', screenidR, 128, rect, [], [], [], [], imagingMode);
    fsm.ifi = Screen('GetFlipInterval', fsm.winL);
    
    MachineUsed = getenv('COMPUTERNAME');
    
    switch (MachineUsed)
        case 'BEHAVCOMP' %%%%%% ------ remove this when M is back-------%%%%%%
    load ('M:\Data\FSM_behaviourboxroom\gamma widefield setup\GammaTable_Dell_E2313.mat')
    Screen('LoadNormalizedGammaTable', fsm.winL, GammaTable_Dell_E2313'*[1 1 1]);
    Screen('LoadNormalizedGammaTable', fsm.winR, GammaTable_Dell_E2313'*[1 1 1]);
%     warning ('Using Wide-Field imaging setup GammaTable')
        otherwise 
    % Load gamma correction table
    load ('M:\Data\Adil\BB2\Matlab\Gamma correction\GammaTable_Dell_2312H.mat')
    Screen('LoadNormalizedGammaTable', fsm.winL, GammaTable_Dell_2312H'*[1 1 1]);
    Screen('LoadNormalizedGammaTable', fsm.winR, GammaTable_Dell_2312H'*[1 1 1]);
%     warning ('Using BehavBoxes GammaTable')
...
%     
    ...
    fsm.gabortex2 = CreateProceduralGabor(fsm.winR, tw, th, 0, [0.5 0.5 0.5 0.0],1,.5);
    ...
    fsm.checkerTextureR = Screen('MakeTexture', fsm.winR, checkerboard);
    ...
    Screen('DrawTexture', fsm.winR, fsm.gabortex2, [], [], tilt, [], [], [], [], kPsychDontDoRotation, [fsm.phase, freq, fsm.sc, contrast, aspectratio, 0, 0, 0]);
