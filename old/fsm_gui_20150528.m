...
% plot image
fsm.handles.ax(1)=axes('Parent',fsm.handles.f,'Units','normalized','Position',[0.65 0.55 0.33 0.4]);
title('Speed (cm/s)');ylim([-10 fsm.spdylim]);
fsm.handles.spdplot = plot(1:fsm.spdxrng,ones(fsm.spdxrng,1),'k','linewidth',2);
...
% start serial port
fsm.ard=serial('COM3','BaudRate',9600); % create serial communication object on port COM7
...
while keeprunning
    ...
    %  spd in   spd out     lick    Tup       Timer   digiOut
        ...
        2           1        1       0        .01        1               ;...% state 1 wait for speed in
    ...
    % send speed averaging bin size and x range
    spdbin = str2num(get(fsm.handles.spdavgbin,'string'))*1000;
    fprintf(fsm.ard,'%s\n',num2str(spdbin));
    fprintf(fsm.ard,'%s',num2str(fsm.spdxrng));
