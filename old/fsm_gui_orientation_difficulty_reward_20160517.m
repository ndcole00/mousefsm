...
function fsm_gui_orientation_difficulty_reward_20160517()
close all
        ...
        if fsm.trialnumcompleted+1 - fsm.blockchangetrial >= str2num(get(fsm.handles.ntrialsperblock,'String'))
            ...
            if fsm.trialnum>0
                if rand < str2num(get(fsm.handles.pprobe,'String')) % if probe trial
                    ...
                    fsm.orientationchange(fsm.trialnum+1) = fsm.difflist(randi(numel([1:length(fsm.difflist)])));%probe is next in list rand
                        while fsm.orientationchange(fsm.trialnum+1) ==  fsm.difflist(fsm.blocktype(fsm.trialnum)); % draw from ori list until the ori does not match main trial ori
                         fsm.orientationchange(fsm.trialnum+1) = fsm.difflist(randi(numel([1:length(fsm.difflist)])));
                        end
                    end
