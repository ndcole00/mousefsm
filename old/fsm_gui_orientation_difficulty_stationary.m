function fsm_gui_orientation_difficulty_stationary()
close all
...
uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit','Enable','inactive',...
    'Position', [0.02 0.005 0.2 0.04],...
    'String','Lever threshold','FontSize',10);
fsm.handles.leverthreshold = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit',...
    'Position', [0.23 0.005 0.1 0.04],...
    'String',fsm.leverthreshold,'FontSize',10);
...
%     end
    ...
    %  spd in   spd out     lick    lever   Tup       Timer   digiOut
        0           0        0       0       1         0.01     0               ;...% state 0 init
        1           1        1       2       1         100      0               ;...% state 1 wait for lever press
        2           2        2       2       3         cueD     0               ;...% state 2 wait for cue
        3           3        3       3       4         cueT     Cue             ;...% state 3 cue on
        4           4        lok     4       5         stimD    0               ;...% state 4 wait for stim
        5           5        lok2    5       10        stimT    Stim            ;...% state 5 stim on, wait for orientation change
        6           6        7       6       AR        waitT    Cont            ;...% state 6 orientation change, wait for lick
        ...
        10          10       lok3    10      6         mcvT     Cont            ;...% state 10 min time forced to look at changed orientation
        11          11       11      11      1         pT       Pun             ;...% punishT
        
        ];

    stm (:,6) = round(stm(:,6)*1000); % sec to ms
    rcvd = '';
    ...
    thresholdlick = round((1024/3.3)*str2num(get(fsm.handles.lickthreshold,'string')));
    fprintf(fsm.ard,'%s\n',num2str(thresholdlick));
    
    % send lever threshold; 3.3V is 1024
    thresholdlever = round((1024/3.3)*str2num(get(fsm.handles.leverthreshold,'string')));
    fprintf(fsm.ard,'%s',num2str(thresholdlever));
