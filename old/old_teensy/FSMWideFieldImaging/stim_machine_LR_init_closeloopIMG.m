function stim_machine_LR_init_closeloopIMG

global fsm


fsm.s = daq.createSession('ni');
fsm.s.addDigitalChannel('Dev1', 'Port0/Line1:7', 'InputOnly');%include 1 to make it an input line
...
try
    ...
    if 1 %%%%%% ------ put the correct table in -------%%%%%%
    % Load gamma correction table
    load ('M:\Adil\wide field setup\luminance correction\GammaTable_Widefield.mat')
    Screen('LoadNormalizedGammaTable', fsm.winL, GammaTable_Widefield'*[1 1 1]);
    Screen('LoadNormalizedGammaTable', fsm.winR, GammaTable_Widefield'*[1 1 1]);
    end
...
%     
    ...
    fsm.sc = 450.0; % spatial const of gaussian
    freq = 1/period;
    tilt = 0;
