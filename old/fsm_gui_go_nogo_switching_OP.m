...
function fsm_gui_go_nogo_switching_OP()
...
% Initialise
fsm.comport = 'COM10';%num2str(GetComPort ('Teensy USB Serial'));
fsm.savedir = 'C:\Jasper\Data\AuxRec\FSM\';
fsm.token = 'EP00_B1';
fsm.fname = '';
fsm.spdrnghigh = 120;
fsm.spdrnglow = 0;
...
% fsm.Tcuedelay = .1;
% fsm.Tcue = 0;
fsm.Tspeedmaintainmin = 3.5;
fsm.Tspeedmaintainmeanadd = .5;
...
uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit','Enable','inactive',...
    ...
    'String','Laser power options (%)','FontSize',10);
...
fsm.handles.autorewd = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','checkbox',...
    'Position', [0.35 0.75 0.1 0.04],'String','Auto Reward?',...
    'Value',1,'FontSize',10);

% Only laser
fsm.handles.onlylaser = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','checkbox',...
    'Position', [0.5 0.75 0.1 0.04],'String','Only laser',...
    'Value',0,'FontSize',10);
...
while keeprunning
    ...
    spdT = str2num(get(fsm.handles.Tspeedmaintainmin,'string')) + min([5 exprnd(str2num(get(fsm.handles.Tspeedmaintainmeanadd,'string')))]); %rand added capped at 5s
    ...
    L    = 2^8; %teensy pin 10, signal for optogenetic laser, square pulse. Actual signal sent to laser is analog out PWM
    ...
    if rand <= pL % laser trial
        LR = 12;
        if Stim~=(Odr1+S)&& Stim~=(Odr2+S);Stim = Stim + L;end % no laser on odor
        Rew = Rew + L;
        iStim = iStim + L;
        ...
        set (fsm.handles.laserpower,'String',['Laser power: ' num2str(pwr)]);
...
%  spd in   spd out     lick    Tup       Timer   digiOut   AnalogOut
    ...
    3           3        3       4         stmT     Stim    pwr      ;...% state 3 Stim on, refractory period
    ...
    13          13       13      9         .1       Bln       0      ;...% state 13 Miss
...
global fsm
% hit: 4 to 5, CR 4 to 9, miss 4 to 13 ro 4 to 7  , FA 4 to 8
% 1:hit  2:CR   3:miss   4:FA
if ~isempty(strfind(triallog, '4to5')); fsm.outcome(fsm.trialnum) = 1; fprintf('Hit\n');% hit
elseif ~isempty(strfind(triallog, '4to8')); fsm.outcome(fsm.trialnum) = 4;fprintf('FA\n');% FA
elseif ~isempty(strfind(triallog, '4to9')); fsm.outcome(fsm.trialnum) = 2;fprintf('CR\n');% CR
elseif ~isempty(strfind(triallog, '4to7'))||~isempty(strfind(triallog, '4to13')); fsm.outcome(fsm.trialnum) = 3;fprintf('Miss\n');% miss
end
...
if length(correcttrials)>=5
    plot(fsm.handles.ax(2),length(correcttrials),yplot(end),'ro');
end
...
function [RT,FAT,refractoryLT,changelist] = findreactiontime(triallog)
...
switch blocktype
    case 1 % visual block
        ...
        if rand < str2num(get(fsm.handles.prewd,'String')) % if rewarded trial
            ...
            fsm.orientation(fsm.trialnum+1) = 135;
        else
            fsm.stimtype(fsm.trialnum+1) = 2; % non rewarded vis
            fsm.orientation(fsm.trialnum+1) = 225;
        end
    ...
    case 2 % odour block
        ...
        if rand < str2num(get(fsm.handles.pirrel,'String')) %
            ...
            if rand < .5; fsm.orientation(fsm.trialnum+1) = 135;else fsm.orientation(fsm.trialnum+1) = 225;end
