function fsm_gui_contrast_odour()
close all
...
fsm.spatialfreq = .1;
fsm.temporalfreq = 2;
fsm.attentionlocationlist = {'vis','sound'};
...
%     'String','Volume','FontSize',10);
% fsm.handles.volume = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit',...
%     'Position', [0.53 0.7 0.08 0.04],...
%     'String',fsm.volume,'FontSize',10);
...
while keeprunning 
    ...
    if max(strcmp(fsm.stimmodality{fsm.trialnum+1}, fsm.attentionlocationlist([1]))) % vis
        Cont = 5;Rew = 7;
    elseif max(strcmp(fsm.stimmodality{fsm.trialnum+1}, fsm.attentionlocationlist([2]))) % sound
        Cont = 6;Rew = 8;
    else
        error('somethings wrong')
    end
    ...
    %  spd in   spd out     lick    Tup       Timer   digiOut
        ...
        5           1        lok2    10        stimT    Stim            ;...% state 5 stim on, wait for contrast change
...
if rand < str2num(get(fsm.handles.pmismatch,'String')) % mismatch trial
    ...
    if strcmp(fsm.attentionlocation{fsm.trialnum+1},fsm.attentionlocationlist{1})
        fsm.stimmodality{fsm.trialnum+1} = fsm.attentionlocationlist{2}; 
    elseif strcmp(fsm.attentionlocation{fsm.trialnum+1},fsm.attentionlocationlist{2})
        fsm.stimmodality{fsm.trialnum+1} = fsm.attentionlocationlist{1};
%     elseif strcmp(fsm.attentionlocation{fsm.trialnum+1},fsm.attentionlocationlist{3})
%         fsm.stimmodality{fsm.trialnum+1} = fsm.attentionlocationlist{4};
%     elseif strcmp(fsm.attentionlocation{fsm.trialnum+1},fsm.attentionlocationlist{4})
%         fsm.stimmodality{fsm.trialnum+1} = fsm.attentionlocationlist{3};
    end
else % no mismatch
