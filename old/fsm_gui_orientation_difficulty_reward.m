% 20160421 added an option to choose the added time stim1 drawn from an exponential distribution 
% 20160513 choosing probes randomly from the entire ori list, the probe is not the next one in the list
% 20160517 difficult block is 1 of n options

function fsm_gui_orientation_difficulty_reward()
close all
...
fsm.savedir = strcat('D:\FSMdata\',num2str(datestr(now,'yyyymmdd')),'\');

if ~exist(fsm.savedir,'dir')
    mkdir (fsm.savedir)
end
...
fsm.Tcue = 0;
fsm.Tstimdelaymin = 1;
fsm.Tstimdelaymeanadd = .5;
fsm.Torientationdelaymin = 2;
fsm.Torientationdelaymeanadd = 1;
fsm.Tminorientationview = .15;
...
fsm.ntrialsperblockRewd = 50;
fsm.rewdlist = [.12,.05];
fsm.rewd = [];
fsm.trialnum = 0;
fsm.trialnumcompleted = 0;
fsm.triallog = {};
fsm.lickthreshold = .4;
fsm.RT = [];
fsm.FAT = {};
...
fsm.handles.rewd = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit',...
    ...
    'String',[num2str(fsm.rewdlist(1)) ',' num2str(fsm.rewdlist(2))],'FontSize',10);
...
% long beep
fsm.handles.longbeep = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','checkbox',...
    'Position', [0.02 0.05 0.2 0.04],'String','Beep stays till end?',...
    'Value',0,'FontSize',10);

% exp delay
fsm.handles.expdelay = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','checkbox',...
    'Position', [0.15 0.05 0.2 0.04],'String','Exponential Delay?',...
    'Value',1,'FontSize',10);

% Indicator: Rewd size
fsm.handles.rewdind = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit','Enable','inactive',...
    'Position', [0.23 0.05 0.1 0.04],...
    'String',['Rewd size: ' ],'FontSize',10,'HorizontalAlignment','left');

% Ntrials per block Rewd
uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit','Enable','inactive',...
    'Position', [0.02 0.005 0.2 0.04],...
    'String','N trials per block Reward','FontSize',10);
fsm.handles.ntrialsperblockRewd = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','edit',...
    'Position', [0.23 0.005 0.1 0.04],...
    'String',fsm.ntrialsperblockRewd,'FontSize',10);
...
% Block or trial by trial for reward modulation
fsm.handles.blockORtbtRewd = uicontrol('Parent',fsm.handles.f,'Units','normalized','Style','popupmenu',...
    'Position', [0.35 0.40 0.13 0.04],'String',{'BlocksRewd','Trial by TrialRewd'},...
    'Value',1,'FontSize',10);
...
%try % do autosave if crashes

keeprunning = 1;fsm.stop = 0;
while keeprunning 
    
    if fsm.trialnum==0 || ~any(ismember(fsm.trialoutcome{fsm.trialnum},{'FA','RefLick'}))% refresh only if hit or miss
       ...
       if get(fsm.handles.expdelay ,'Value')==1 
    ...
    else %repeat same params if early lick
        fsm.orientationchange(fsm.trialnum+1)=fsm.orientationchange(fsm.trialnum);
        fsm.orientation(fsm.trialnum+1) = fsm.orientation(fsm.trialnum);   
        fsm.rewd(fsm.trialnum+1) = fsm.rewd(fsm.trialnum);
        if get(fsm.handles.blockORtbt,'Value') ==1;
            fsm.blocktype(fsm.trialnum+1) = fsm.blocktype(fsm.trialnum);
        end
        if get(fsm.handles.blockORtbtRewd,'Value') ==1;
            fsm.blocktypeRewd(fsm.trialnum+1) = fsm.blocktypeRewd(fsm.trialnum);
        end     
    end
    
    lok = 1;% restart on early lick
    lok2 = 11;% punish on early lick
    lok3 = 9;% end on early lick
    ...
    %Rew = 1;
    Stim = 4;
    if find(fsm.rewdlist==rewT) ==1 % first in rewdlist is high tone - should be higher rewd value
        Cue = 2;
    elseif find(fsm.rewdlist==rewT) ==2% second in rewdlist is low tone - should be lower rewd value
        Cue = 3;
    else error('rewd not found in rewdlist')
    end
...
%     end
    ...
    % digiOut mapping: 1-rewd valve; 2-High tone cue; 3-Low tone cue; 
    % 4-stim both sides; 5-orientation change; 6-freeze
    % 7-rewd+change; 
    ...
    %  spd in   spd out     lick    Tup       Timer   digiOut
        ...
        2           2        2       3         cueD     0               ;...% state 2 wait for cue
        3           3        3       4         cueT     Cue             ;...% state 3 cue on
        4           4        lok     12        stimD    Cu2             ;...% state 4 wait for stim
        5           5        lok2    13        stimT    Stim            ;...% state 5 stim on, wait for orientation change
        6           6        14      AR        waitT    Cont            ;...% state 6 orientation change, wait for lick
        ...
        11          11       11      9         pT       Pun             ;...% state 11 punishT
        
        12          12       12      5         .005     Stim            ;...% state 12 delay
        13          13       13      10        .005     Cont            ;...% state 13 delay
        14          14       14      7         .005     Cont            ;...% state 14 delay
        ];
    ...
    switch rcvd
        ...
        case 'startingFSM'
            ...
            set(fsm.handles.trialnum,'String',['Trial Number: ' num2str(fsm.trialnum) '(' num2str(fsm.trialnumcompleted) ')'])
            ...
            while trialend == 0%~strcmp(rcvd2,'E'); % it will send an E when trial ends
                ...
                stim_machine_LR_orientation_freeze_sound
                ...
                if fsm.ard.BytesAvailable
                    ...
                    if rcvd2 == 'E';
                        ...
                        [fsm.RT(fsm.trialnum), fsm.FAT{fsm.trialnum}, fsm.refractoryLT{fsm.trialnum},changelist,trialoutcome]...
                            = findrectiontime(fsm.triallog{fsm.trialnum});
                        ...
                        fsm.trialoutcome{fsm.trialnum} = trialoutcome;
                        fprintf('%s\n',trialoutcome);
                        if ismember(trialoutcome,{'Hit','Miss'}); fsm.trialnumcompleted = fsm.trialnumcompleted +1;end
                        updateRTplot
...
try Screen('CloseAll');PsychPortAudio('Close',fsm.pahandle);
...
function [RT,FAT,refractoryLT,changelist,trialoutcome] = findrectiontime(triallog)
trialoutcome = 'Miss';%unless changed below
% orientation change 5 to 13, lick 6 to 14
ct1 = strfind(triallog, '_5to13');
if ~isempty(ct1)
    ct2 = strfind(triallog(1:ct1(end)-1),'_');
    ct = str2num(triallog(ct2(end)+1:ct1(end)-1));
    
    lt1 = strfind(triallog, '_6to14');
    if ~isempty(lt1)
...
%False Alarm Time
% stim on 4 to 12, lick 5 to lok2(11 or 1)
lstring = '_5to11';
...
for f = 1:length(lt1)
    ...
    st1 = strfind(triallog(1:lt1(f)-1), '_4to12');
...
% stim on 4 to 12, lick 10 to lok3 (9)
lstring = '_10to9';
lt1 = strfind(triallog, lstring);
...
if ~isempty (curr)% put this bec was crashing here
    ...
    if strcmp(fsm.trialoutcome{fsm.trialnum},'Hit'); % Hit 
        mrkr = '.';  
        RT = fsm.RT(end);
        plot(fsm.handles.ax(2),fsm.trialnumcompleted,RT,'color',cmrkr,'marker',mrkr);
    end
    
    if strcmp(fsm.trialoutcome{fsm.trialnum},'Miss'); % miss
        mrkr = '^';  
        plot(fsm.handles.ax(2),fsm.trialnumcompleted,str2num(get(fsm.handles.Trewdavailable,'string')),'color',cmrkr,'marker',mrkr);
    end
    c = jet(10);
    
    if strcmp(fsm.trialoutcome{fsm.trialnum},'FA');    % false alarms
        mrkr = '.';
    ...
    if strcmp(fsm.trialoutcome{fsm.trialnum},'RefLick');    % refractory period licks
        mrkr = '*';
...
fsm.difflist = str2num(get(fsm.handles.orientationchangelist,'String'));
% first entry is easy, rest n are difficult; in difficult block choose 1 rand from n

switch blockORtbt % difficuty
    case 1 % blocks
        ...
        if fsm.trialnumcompleted+1 - fsm.blockchangetrial >= str2num(get(fsm.handles.ntrialsperblock,'String'))
            fsm.blockchangetrial = fsm.trialnumcompleted+1;
            %curr = find(fsm.difflist==fsm.orientationchange(fsm.trialnum));
            if fsm.blocktype(fsm.trialnum) == 2;% if difficult% length(fsm.difflist)
                fsm.orientationchange(fsm.trialnum+1) = fsm.difflist(1);
                fsm.blocktype(fsm.trialnum+1) = 1;
            else % if easy change to diff
                fsm.orientationchange(fsm.trialnum+1) = fsm.difflist(randi([2, length(fsm.difflist)]));% fsm.difflist(fsm.blocktype(fsm.trialnum)+1);
                fsm.blocktype(fsm.trialnum+1) = 2; % fsm.blocktype(fsm.trialnum) + 1;
            end
            ...
            if fsm.trialnum>0
                if rand < str2num(get(fsm.handles.pprobe,'String')) % if probe trial
                    if fsm.blocktype(fsm.trialnum) == 2;% diff block
                        fsm.orientationchange(fsm.trialnum+1) = fsm.difflist(1);%probe is first in list (easy)
                    else % easy block
                        fsm.orientationchange(fsm.trialnum+1) = fsm.difflist(randi([2, length(fsm.difflist)]));%probe is from diff
                    end
                else % non probe normal trial
                    if fsm.blocktype(fsm.trialnum) == 2 % diff block
                        fsm.orientationchange(fsm.trialnum+1) = fsm.difflist(randi([2, length(fsm.difflist)])); % fsm.difflist(fsm.blocktype(fsm.trialnum));
                    else % easy
                        fsm.orientationchange(fsm.trialnum+1) = fsm.difflist(1);
                    end
                end
...
%%%%% for choosing reward amount
blockORtbtRewd = get(fsm.handles.blockORtbtRewd,'Value');
fsm.rewdlist = str2num(get(fsm.handles.rewd,'String'));% list of rewd values
if length(fsm.rewdlist)>2; error('no more than 2 rewd values');end
switch blockORtbtRewd % rewd
    case 1 % blocks
        if isempty(fsm.rewd) % first trial
            fsm.rewd(1) = fsm.rewdlist(1);
            fsm.blocktypeRewd(1) = 1;
        end
        % is it time to change blocks?
        if fsm.trialnumcompleted+1 - fsm.blockchangetrialRewd >= str2num(get(fsm.handles.ntrialsperblockRewd,'String'))
            fsm.blockchangetrialRewd = fsm.trialnumcompleted+1;
            %curr = find(fsm.difflist==fsm.orientationchange(fsm.trialnum));
            if fsm.blocktypeRewd(fsm.trialnum) == length(fsm.rewdlist)
                fsm.rewd(fsm.trialnum+1) = fsm.rewdlist(1);
                fsm.blocktypeRewd(fsm.trialnum+1) = 1;
            else
                fsm.rewd(fsm.trialnum+1) = fsm.rewdlist(fsm.blocktypeRewd(fsm.trialnum)+1);
                fsm.blocktypeRewd(fsm.trialnum+1) = fsm.blocktypeRewd(fsm.trialnum) + 1;
            end
            
        else
            if fsm.trialnum>0 % not implementing probes in Rewd now
%                 if rand < str2num(get(fsm.handles.pprobe,'String')) % if probe trial
...
%                     end
%                 else % non probe normal trial
                    fsm.rewd(fsm.trialnum+1) = fsm.rewdlist(fsm.blocktypeRewd(fsm.trialnum));
%                 end
                fsm.blocktypeRewd(fsm.trialnum+1) = fsm.blocktypeRewd(fsm.trialnum);
            end
        end
    ...
    case 2 % trial by trial
        rr = randi([1, length(fsm.rewdlist)]);
        fsm.rewd(fsm.trialnum+1) = fsm.rewdlist(rr);
end
set(fsm.handles.rewdind,'String',['Rewd size: ' num2str(fsm.rewd(fsm.trialnum+1))])
