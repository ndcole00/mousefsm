...
while keeprunning   
     ...
     if fsm.trialnum ==0   
         ...
         if ~any(ismember(fsm.trialoutcome{fsm.trialnum},{'FA','RefLick'}))% refresh only if hit or miss
...
%     end
    ...
    %  spd in   spd out     lick    Tup       Timer   digiOut
        ...
        4           4        lok     20        stimD    Cu2             ;...% state 4 wait for stim
        5           5        lok2    21        stimT    Stim            ;...% state 5 stim on, wait for orientation change
        6           6        22      AR        waitT    Cont            ;...% state 6 orientation change, wait for lick
        ...
        11          11       11      9         pT       Pun             ;...% punishT
        
        20          20       20      5         .005     Stim            ;...% delay
        21          21       21      10        .005     Cont            ;...% delay
        22          22       22      7         .005     Cont            ;...% delay
        ];
...
% orientation change 5 to 21, lick 6 to 22
ct1 = strfind(triallog, '_5to21');
if ~isempty(ct1)
    ct2 = strfind(triallog(1:ct1(end)-1),'_');
    ct = str2num(triallog(ct2(end)+1:ct1(end)-1));
    
    lt1 = strfind(triallog, '_6to22');
    if ~isempty(lt1)
...
%False Alarm Time
% stim on 4 to 20, lick 5 to lok2(11 or 1)
lstring = '_5to11';
...
for f = 1:length(lt1)
    ...
    st1 = strfind(triallog(1:lt1(f)-1), '_4to20');
...
% stim on 4 to 20, lick 10 to lok3 (9)
lstring = '_10to9';
lt1 = strfind(triallog, lstring);
