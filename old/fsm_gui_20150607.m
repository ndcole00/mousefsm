            ...
            while trialend == 0%~strcmp(rcvd2,'E'); % it will send an E when trial ends
                ...
                if fsm.ard.BytesAvailable
                    ...
                    if rcvd2 == 'E';
                        trialend = 1;
                        % read trial log
                        while ~fsm.ard.BytesAvailable;fprintf('waiting\n');end
                        stopsignal = '';triallog = '';
                        while ~strcmp(stopsignal,'End')
                            partialtriallog = fscanf(fsm.ard,'%s');
                            triallog = strcat(triallog,partialtriallog);
                            stopsignal = partialtriallog(end-2:end);
                        end
...
stopsignal = '';triallog = '';
while ~strcmp(stopsignal,'stopped')
    partialtriallog = fscanf(fsm.ard,'%s');
    triallog = strcat(triallog,partialtriallog);
    try;stopsignal = partialtriallog(end-6:end);end
end
fsm.triallog{fsm.trialnum} = triallog;
