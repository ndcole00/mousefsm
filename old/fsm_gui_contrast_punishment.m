function fsm_gui_contrast_punishment()
close all
...
fsm.Tcuedelay = .1;
fsm.Tcue = 0;
fsm.Tstimdelaymin = 1.5;
fsm.Tstimdelaymeanadd = .5;
fsm.Tcontrastdelaymin = .3;
fsm.Tcontrastdelaymeanadd = 0;
fsm.Tmincontrastview = .25;
...
while keeprunning 
    ...
    if max(strcmp(fsm.contrastchangelocation{fsm.trialnum+1}, fsm.attentionlocationlist([1 3]))) % left or front
        Cont = 5;Rew = 7;
    elseif max(strcmp(fsm.contrastchangelocation{fsm.trialnum+1}, fsm.attentionlocationlist([2 4]))) % right or back
        Cont = 6;Rew = 8;
    else
        error('somethings wrong')
    end
    ...
    % 4-stim both sides; 5-left contrast change; 6-right contrast change
    ...
    %  spd in   spd out     lick    Tup       Timer   digiOut
        ...
        4           1        lok2    5         stimD    0               ;...% state 4 wait for stim
        5           1        lok     10        stimT    Stim            ;...% state 5 stim on, wait for contrast change
