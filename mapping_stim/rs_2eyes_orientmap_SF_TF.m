function rs_2eyes_orientmap_SF_TF
...
% 1=spatial  frequency mapping with temporal frequency fixed at 2Hz
% 2=temporal frequency mapping with spatial frequency fixed at 0.04Hz
EXPTYP = 2; 
if EXPTYP==1,       
    STM.orient = [0,45];   
    %STM.FreqSpatial  = [0.02,0.04,0.08,0.16,0.32]; % cyc/deg
    STM.FreqSpatial  = [0.02,0.04,0.08,0.16]; % cyc/deg
    %STM.FreqTemporal = [0.5, 1, 2, 4, 8, 15, 24]; % Hz
    STM.FreqTemporal = [0.5, 1, 2, 4, 8, 16]; % Hz    
elseif EXPTYP==2,
    STM.orient       = [0 45 90 135 180 225 270 315];
    STM.FreqSpatial  = [0.02,0.04,0.08,0.16]; % cyc/deg
    STM.FreqTemporal = [2]; % Hz
    %STM.FreqTemporal = [16]; % Hz
end

if 0,    
    % combi SF x TF 48 conditions
    % rs_2eyes_orientmap_SF_TF: direction SF x TF
    S=2*4*6*10*3.5; % condition types x ntrl x trialtime 480 trials
    %S=1*4*6*10*3.5; % condition types x ntrl x trialtime
    fprintf('%dmin %dsec\n',floor(S/60),(S-60*floor(S/60)))    
    
    % rs_2eyes_orientmap_SF_TF: direction x SF 
    S=8*4*1*10*3.5; % condition types x ntrl x trialtime 320 trials
    %S=8*4*6*4*3.5; % condition types x ntrl x trialtime    
    fprintf('%dmin %dsec\n',floor(S/60),(S-60*floor(S/60)))
    
    % rs_2eyes_retinotopy_fine
    S=12*9*10*2*0.3%*2; % condition types x ntrl x trialtime x on/off
    %S=16*12*10*0.3*2; % condition types x ntrl x trialtime x on/off
    fprintf('%dmin %dsec\n',floor(S/60),(S-60*floor(S/60)))
    
    % rs_2eyes_orientmap
    S=8*10*3.5; % condition types x ntrl x trialtime x on/off
    %S=16*12*10*0.3*2; % condition types x ntrl x trialtime x on/off
    fprintf('%dmin %dsec\n',floor(S/60),(S-60*floor(S/60)))
    
    % rs_2eyes_retinotopy_flicker
    S=4*3*2*10; % condition types x ntrl x trialtime x on/off
    %S=16*12*10*0.3*2; % condition types x ntrl x trialtime x on/off
    fprintf('%dmin %dsec\n',floor(S/60),(S-60*floor(S/60)))
    
%     23min 20sec
%     18min 40sec
%     10min 48sec % retinotopy
%     4min 40sec
%     4 min 0sec
%     15 % playback around 15 min
...
if ~DEMO
    ...
    stem    = sprintf('rs_2eyes_orientmap_SF_TFe%d',EXPTYP);
...
try,
    ...
    % length(STM.FreqSpatial)*length(STM.FreqTemporal)
    clear GAB;
    for i=1:length(STM.FreqSpatial),
        GAB{i}.SF       = STM.FreqSpatial(i);
        ...
        for j=1:length(STM.FreqTemporal),
            GAB{i}.Cycpersec(j)    = STM.FreqTemporal(j);
        ...
        end % for j=1:length(STM.FreqTemporal),
    end % for i=1:length(STM.FreqSpatial),
    ...
    for i=1:length(STM.FreqSpatial),
        ...
        %snw=square(2*pi*(1/GAB{i}.Period)*X+GAB{i}.Phase);
        %
    ...
    for i=1:length(STM.orient),
        for j=1:length(STM.FreqSpatial),
            for k=1:length(STM.FreqTemporal),
            z = z+1;
            STM.C(z,:)=[i,j,k];            
            end
        end
    ...
    LOG.runstim      = 'rs_2eyes_orientmap_SF_TF';    
    ...
    while ~Par.ESC,
        ...
        cur.ori =STM.C(cur.cnd,1);
        cur.frqs=STM.C(cur.cnd,2);
        cur.frqt=STM.C(cur.cnd,3);
        
        fprintf('%3d: word = %3d, cnd = %3d, ori=%3d,sfrq=%3d,tfrq=%3d\n',TZ,cur.word,cur.cnd,cur.ori,cur.frqs,cur.frqt);
        ...
        for i=2:TIMING.EVENT2_FRAMES,
            ...
            if SHOWTEXT,
                ...
                DrawFormattedText(expWin,sprintf('LFT:wrd=%d,cnd=%d,xoffset1=%1.2f,ori=%d,sfrq=%1.2f,tfrq=%1.2f',cur.word,RotAngle1,xoffset1,RotAngle1,STM.FreqSpatial(cur.frqs),STM.FreqTemporal(cur.frqt)), 'center', 'center'); % help DrawFormattedText
            end
