...
function fsm_gui_go_nogo_switching_combined_widefield_opto()
...
% start serial port
fsm.comport = 'COM5'; % Overwrite in the case of some error
...
elseif strmatch (fsm.version,'USBcom')
    stim_machine_init_go_nogo_switching_USBcom_widefield_opto
end
...
while keeprunning
    ...
    % if laser trial
    if rand < pL  && ~fsm.itiLaser && ~fsm.constantLaser && strcmpi(fsm.handles.alternateLaser.String{get(fsm.handles.alternateLaser','value')}, 'Off')
        LR = 12;
        ...
        if fsm.fullScreenOP
            fsm.optoImages = prepareNextStimulus(fsm.optoImages);
        end
...
switch rcvd
    ...
    case 'startingFSM'
        ...
        while trialend == 0
            ...
            elseif strmatch(fsm.version,'USBcom')
                stim_machine_go_nogo_switching_USBcom_widefield_opto
            end
