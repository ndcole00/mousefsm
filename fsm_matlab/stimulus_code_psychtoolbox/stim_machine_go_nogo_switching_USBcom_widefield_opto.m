function stim_machine_go_nogo_switching_USBcom_widefield_opto
dbstop if error
...
switch trigg
    ...
    case 0 % gray screen
        ...
        if ~fsm.grayscreen
            ...
            if fsm.fullScreenOP
                % Colour the widefield optogenetic screen black when not
                % preseneting stimuli
                Screen('FillRect', fsm.winOP, [0 0 0]);
            end
    ...
    case 1 % Stim 1
        ...
        if fsm.fullScreenOP
            % Draw the next optogenetic stimulus to the screen if it's a
            % laser trial
            if fsm.laserpower(fsm.trialnum) ~= 0
                Screen('DrawTexture', fsm.winOP, fsm.optoImages.imageTextures{fsm.optoImages.currentImageID}, [], fsm.optoImages.destRects{fsm.optoImages.currentImageID}, 0);
            else
                Screen('FillRect', fsm.winOP, [0 0 0]);
            end
        end
