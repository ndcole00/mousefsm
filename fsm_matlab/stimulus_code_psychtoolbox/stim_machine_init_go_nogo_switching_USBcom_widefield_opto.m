function stim_machine_init_go_nogo_switching_USBcom_widefield_opto
...
try
    ...
    % Todo: Add opto screen as an option for the fetch screens app
    if length(allscreens) > 4
        % Hardcoding for now
        screenOpto = 4;
%         screenOpto = screenIDs(2);
        
        % Set up the opto stimuli
        % Open a fullscreen onscreen window on that display, choose a background
        % color of black
        [fsm.winOP,screenRectOP] = Screen('OpenWindow', screenOpto, 0, [], [], [], [], [], 0);
        
        % Make the stimuli for the optogenetic stim
        opSettings = struct();
        opSettings.imagesLocation = 'C:\Users\Khan Lab\Documents\MATLAB\MouseFSM\fsm_matlab\Widefield_opto\cropped_Images';
        opSettings.imageHeight = screenRectOP(4);
        opSettings.Xoffset = 0;
        opSettings.Yoffset = 0;
        opSettings.screenRect = screenRectOP;
        opSettings.psuedorandom = false;
        opSettings.psychWindow = fsm.winOP;
        fsm.optoImages = lightBox(opSettings);
        fsm.fullScreenOP = true;
    else
        fsm.fullScreenOP = false;
    end
    ...
    if 1
        ...
        elseif strcmp(getenv('computername'),'BEHAVIOUR-BOX-A') % Box A
            load ('C:\Users\Behaviour Only A\Documents\MATLAB\mousefsm\gamma correction\gamma correction KCL 2p\calib20190430_DellU2715Hc_bright50_cont50.mat')
        elseif strcmp(getenv('computername'),'BEHAVIOUR-BOX-B') % Box B
            load ('C:\Users\Behaviour-Box-B\Documents\MATLAB\mouseFSM\gamma correction\gamma correction KCL 2p\calib20190430_DellU2715Hc_bright50_cont50.mat')
        elseif strcmp(getenv('computername'),'BEHAVIOUR-BOX-C') % Box C
            load ('C:\Users\Behaviour Only One\Documents\MATLAB\mousefsm\gamma correction\gamma correction KCL 2p\calib20190430_DellU2715Hc_bright50_cont50.mat')
        elseif strcmp(getenv('computername'),'BEHAVIOUR-BOX-D') % Box D
            load ('C:\Users\Behaviour Only D\Documents\MATLAB\mousefsm\gamma correction\gamma correction KCL 2p\calib20190430_DellU2715Hc_bright50_cont50.mat')
        end
        
        load ('C:\Users\Khan Lab\Documents\MATLAB\MouseFSM\gamma correction\gamma correction KCL 2p\calib20190430_DellU2715Hc_bright50_cont50.mat')
